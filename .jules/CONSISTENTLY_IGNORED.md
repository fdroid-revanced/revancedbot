...
**- Pattern:** Bumping GitHub Actions to non-existent versions (e.g., `actions/checkout@v6`), downgrading dependencies without explicit request (e.g., `actions/checkout@v4`), or downgrading tools in `mise.toml`.
...
## IGNORE: Swallowing Exceptions in Except Blocks

**- Pattern:** Using empty `except` blocks or catching exceptions without utilizing the centralized error reporting module (e.g., `logger.error` directly or `pass`).
**- Justification:** Swallowed exceptions mask critical bugs and upstream errors. All unexpected exceptions must be routed through the centralized error reporting handler (e.g. `revancedbot/errors.py:report_error()`).
**- Files Affected:** `revancedbot/**/*.py`, `src/revancedbot/**/*.py`
